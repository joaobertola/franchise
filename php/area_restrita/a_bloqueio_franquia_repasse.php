	...
	$abre_form = $_REQUEST['abre_form'];
		...
		for ($i = $inicio; $i <= $fim; $i++) {
			...
			if ($i == $inicio) {
				...
				if($sel == "01"){
					$_ano_esc = $ano_esc - 1;
					$periodo .= date("d/m/".$ano_esc, $ts) . " 18:00:01 ";
				}else{
					$periodo .= date("d/m/".$ano_esc, $ts) . " 18:00:01 ";
				}	
			}elseif ($i == $fim) {			
				$ultimo = date($ano_esc."-m-d",$ts);
				$periodo .= 'at&eacute; '.date("d/m/".$ano_esc, $ts) . " 18:00:00 <br>";
			}							
		}
...
<table border='0' width='65%' align='center' cellpadding='0' cellspacing='1' style='border: 1px solid #D1D7DC; background-color:#FFFFFF'>
	<tr>
		<td align='center' height='40'>RELATORIO DE BLOQUEIO DE FRANQUIAS - REPASSE<br></td>
	</tr>
	<tr>
...
if ( mysql_num_rows($res) > 0 ){
	while ( $reg = mysql_fetch_array($res) ){
		...
		$qry_age = mysql_query($sql_age,$con);
		...
		if ( $bloqueio == 'S'){
			$sql_update = "UPDATE cs2.franquia SET situacao_repasse = 1 WHERE id = $id_franquia";
		}else{
			$sql_update = "UPDATE cs2.franquia SET situacao_repasse = 0 WHERE id = $id_franquia";
		}
		$res_update = mysql_query($sql_update,$con);
	}
}
$html .= "<tr><td>Data / Hora do Processamento: $data_hora</td></tr>";
$html .= "</table>";

$data_hora = date("d/m/Y h:m:s");

echo "Procedimento: BLOQUEIO DE FRANQUIAS REPASSE - Finalizado as $data_hora : Periodo; $datai a $dataf";
...
try {
	...
	$mail->AddAddress("administrativo@webcontrolempresas.com.br","Administrativo - Web Control Empresas");
	...
	$mail->Subject = "Bloqueio Automatico de Franquias - REPASSE"; //ASSUNTO DA MENSAGEM
