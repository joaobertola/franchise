...
while ( $reg = mysql_fetch_array( $qry ) ){
	...
	if ( $saldo_crediario < 0 ){
		...
		if ( $sitcli == 2 ) $lista_canc[] = $logon.';'.$nome.';'.$st_ctr.';'.$saldo_crediario;
		else $lista_ativ[] = $logon.';'.$nome.';'.$st_ctr.';'.$saldo_crediario;
...
?>
	...
	<tr>
		<td colspan="5">Total Debito Contratos Cancelados: (<?=$qtd_pc?>) R$ <?=$total_pc?><hr></td>
	</tr>
</table>
...
<table align='center' width='800' border='0' cellpadding='0' cellspacing='1' class='bodyText'>
	...
	<tr>
		<td colspan="5">Total Debito Ativos: (<?=$qtd_pa?>) R$ <?=$total_pa?><hr></td>
	</tr>
    <tr>
