<?php
	...
	// HEADER DO LOTE
	$Registro  = '341';                               // 001 a 003 - Codigo do Banco
	...
	$Registro .= colocaespacos('',10);                // 133 a 142 - Historico de conta corrente
	...
	if ( $qtd > 0 ){
		while($registro = mysql_fetch_array($qr_sql)){
			...
			$Registro = '341';                                         // 001 a 003 - Codigo do banco
			$Registro .= str_pad($NumeroLote, 4, 0, STR_PAD_LEFT);     // 004 a 007 - Numero do lote
			$Registro .= '3';                                          // 008 a 008 - Tipo do registro: Registro detalhe do lote
			$Registro .= str_pad($NumeroRegistro, 5, 0, STR_PAD_LEFT); // 009 a 013 - Numero sequencial do registro no lote
			$Registro .= 'A';                                          // 014 a 014 - Codigo do segmento do registro detalhe
			$Registro .= '000';                                        // 015 a 017 - Codigo da instrucao p/ movimento  00 - Inclusao
			$Registro .= '000';                                        // 018 a 020 - ZEROS
			$Registro .= str_pad($banco_cliente, 3, 0, STR_PAD_LEFT);  // 021 a 023 - Codigo do Banco do Favorecido
			...
			$Registro .= '00000000';                                   // 155 a 162 - Data real da efetivacao do lancamento
			$Registro .= '000000000000000';                            // 163 a 177 - Valor real da efetivacao do lancamento
			$Registro .= str_pad($CODLOJA, 18, ' ', STR_PAD_RIGHT);    // 178 a 195 - Outras informacoes
	...
	$Registro  = '341';                                    // 001 a 003 - Codigo do banco
	$Registro .= colocazeros($NumeroLote,4);               // 004 a 007 - Numero do lote
	...
	$valor = str_replace(',','',$valor);
	$valor = str_replace('.','',$valor);
	$Registro .= colocazeros($valor,18);                   // 024 a 041 - Somatoria dos valores
	$Registro .= '000000000000000000';                     // 042 a 059 - ZEROS
	$Registro .= colocaespacos('',171);                    // 060 a 230 - BRANCOS
	$Registro .= colocaespacos('',10);                     // 231 a 240 - Codigo das ocorrencias para retorno
	$totlinha++;
	$registros .= "$Registro\n";
	...
	$Registro .= '20'.$forma_pgto;                    // 033 a 036 - Identificacao do lancamento
	$Registro .= colocaespacos('',16);                // 037 a 052 - Em branco
    ...
    $sql = "SELECT  DISTINCT(a.id_franquia) id, mid(b.razaosoc,1,30) razaosoc, b.banco,
            ...
            FROM cs2.cadastro_emprestimo_franquia a
            INNER JOIN cs2.franquia b ON b.id = a.id_franquia
            WHERE b.banco <> '341' AND b.situacao_repasse = 0 AND a.depositado_cta_cliente = 'N'
			GROUP BY a.protocolo
			ORDER BY banco,tpconta,titular";
	$qr_sql = mysql_query($sql,$conexao) or die("Erro no SQL: $sql");
	$qtd = mysql_num_rows($qr_sql);
	...
	$valor = str_replace(',','',$valor);
	$valor = str_replace('.','',$valor);
	$Registro .= colocazeros($valor,18);                   // 024 a 041 - Somaturia dos valores
	$Registro .= '000000000000000000';                     // 042 a 059 - ZEROS
	$Registro .= colocaespacos('',171);                    // 060 a 230 - BRANCOS
	$Registro .= colocaespacos('',10);                     // 231 a 240 - Codigo das ocorrencias para retorno
	$totlinha++;
	...
	$totlinha++;
	$Registro  = '341';                        // 001 a 003 - Codigo do banco
	$Registro .= '9999';                       // 004 a 007 - Nomero do lote
