...
jQuery.imgAreaSelect.init=function(img,options){var $area=jQuery('<div></div>'),
...
moveY,
resizeMargin=10,
resize=[],V=0,H=1,keyDown,d,aspectRatio,x1,x2,y1,y2,x,y,
...
function adjust(){imgOfs=jQuery(img).offset();imgWidth=jQuery(img).width();
imgHeight=jQuery(img).height();
if(jQuery(parent).is('body'))parOfs=parScroll={left:0,top:0};
else{parOfs=jQuery(parent).offset();
parScroll={left:parent.scrollLeft,top:parent.scrollTop}}
left=viewX(0);top=viewY(0)}
function update(resetKeyPress){$a.css({left:viewX(selection.x1)+'px',
