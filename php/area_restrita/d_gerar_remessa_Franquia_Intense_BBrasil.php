<?php
	...
	function EnvioArqBco($Tipo,$NumeroArquivo,$NumeroLote){
		...
		if ( $Tipo == 'HEADER' ){
			...
			$Registro .= '12275138000182';                 // 019 a 032 - N�mero de inscri��o do cedente
			$Registro .= '0009782620126       ';           // 033 a 052 - C�digo do conv�nio no banco
			$Registro .= '03051';                          // 053 a 057 - C�digo da ag�ncia do cedente
			$Registro .= '1';                              // 058 a 058 - D�gito da ag�ncia do cedente
			$Registro .= colocazeros('23095',12);          // 059 a 070 - N�mero da conta do cedente
			$Registro .= '2';                              // 071 a 071 - D�gito da conta do cedente
			$Registro .= ' ';                              // 072 a 072 - D�gito verificador da ag�ncia / conta
			$Registro .= colocaespacosdir('INTENSE SOLUCOES EM TECNOLOGIA',30);// 073 a 102 - Nome do cedente
			...
			$Registro .= '03051';                             // 053 a 057 - C�digo da ag�ncia do cedente
			$Registro .= '1';                                 // 058 a 058 - D�gito da ag�ncia do cedente
			$Registro .= colocazeros('23095',12);             // 059 a 700 - N�mero da conta do cedente
			$Registro .= '2';                                 // 071 a 071 - D�gito da conta do cedente
		...
		for ( $I=1 ; $I <= $reg; $I++ ){
			...
			if ( $TpConta == '1' ){
				if ( ( $saldo - 8 ) > 0 ){
					...
					$NumeroRegistro++;
					$TipoLcto = '';
					$registro .= EnvioArqBco_DETALHES('DETALHES_LOTE_DOC_SEGMENTO_A',$NumeroLote,$NumeroRegistro,$Banco,$Agencia,$Conta,$NomeFavorecido,$IdFranquia,($saldo-8),'CC_OB',$DocFavorecido);
					$totccou  += ($saldo-8);
					$totlinha++;
					$NumeroRegistro++;
		...
		for ( $I=1 ; $I <= $reg; $I++ ){
			...
			if ( $TpConta == '2' ){
				if ( ( $saldo - 8 ) > 0 ){
					...
					$NumeroRegistro++;
					$TipoLcto = '';
					$registro .= EnvioArqBco_DETALHES('DETALHES_LOTE_DOC_SEGMENTO_A',$NumeroLote,$NumeroRegistro,$Banco,$Agencia,$Conta,$NomeFavorecido,$IdFranquia,($saldo-8),'CP_OB',$DocFavorecido);
					$totcpou  += ($saldo-8);
					$totlinha++;
					$NumeroRegistro++;
