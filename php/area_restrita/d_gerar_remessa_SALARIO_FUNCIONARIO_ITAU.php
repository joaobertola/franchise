<?php
	...
	*/
    ...
    $sql = "SELECT id, nome, nr_banco, agencia, tp_conta, conta, cpf, (salario-adiantamento) as salario, vt
			FROM cs2.funcionario
			WHERE nr_banco = '341' AND tp_conta = 'C' AND ativo = 'S' AND id_empregador = 2
			ORDER BY nome";
    ...
    $sql = "SELECT id, nome, nr_banco, agencia, tp_conta, conta, cpf, (salario-adiantamento) as salario, vt
			FROM cs2.funcionario
			WHERE nr_banco = '341' AND tp_conta = 'P' AND ativo = 'S' AND id_empregador = 2
			ORDER BY nome";
	$qr_sql = mysql_query($sql,$conexao) or die("Erro no SQL: $sql");
	$qtd = mysql_num_rows($qr_sql);
	if ( $qtd > 0 ){
    ...
    $sql = "SELECT id, nome, nr_banco, agencia, tp_conta, conta, cpf, (salario-adiantamento) as salario, vt
			FROM cs2.funcionario
			WHERE nr_banco <> '341' AND salario >= 5000 AND nr_banco > 0 AND ativo = 'S' AND id_empregador = 2
			ORDER BY nome";
	...
	include( "d_gerar_remessa_VTVR_FUNCIONARIO_ITAU.php" );
