...
class RPS
{
    ...
    public $URLwebservice = array('producao' => 'http://200.195.154.239/WebEloWSIss/nfse_v1_2.wsdl',
                                  'homologacao' => 'http://200.195.154.239/WebEloWSIss/nfse_v1_2.wsdl');
...
SQLQUERY;
    ...
    private function __assinarRPS( $returned_content  )
    {
        ...
        try
        {
            ...
            if ( !$dom->schemaValidate ( PATH_SCHEMAS . 'nfse.xsd' ) ):
                    echo '999;XML do rps inválido, verifique o arquivo  ! ['.PATH_SCHEMAS . 'nfse.xsd]';
                    exit;
            endif;
            ...
            foreach ( $ListaRps->getElementsByTagName ( "Rps" ) as $Rps ):
                ...
                #$newNode = $dom->createElement('X509SubjectName', $this->SubjectName );
            ...
            //SignatureValue
            $newNode = $dom->createElement('SignatureValue',$signatureValue);
            #$newNode->setAttribute('ID',$idLoteRps);
            $Signature->appendChild($newNode);
...
SQLQUERY;
            ...
            if( empty($rs) ):	
                ...
                VALUES
                        (:id_venda, :tipo_nota, :status, :numero_nota, :ambiente_nf, null, :lote, :arquivo)";
                $amb = $this->aplicativo;
                if ( $amb == 'producao ' ) $amb = 2;
                else $amb = 1;
                try
                {
                    $stmt = DB::connect()->Prepare ( $sql );
                ...
                // Atualizando Lote do Rps
                $amb = $this->aplicativo;
                if ( $amb == 'producao ' ) $amb = 'P';
                else $amb = 'H';
                ...
                if( empty($rs) ):
                    ...
                    {
                        $stmt = DB::connect()->Prepare ( $sql );
                        $stmt->bindValue(':id_cadastro', $this->NumeroPedido, PDO::PARAM_INT);
                        $stmt->bindValue(':ambiente', $amb , PDO::PARAM_INT);
    ...
    public function ValidarXml()
    {
        try{
            ...
            if( $xmlRecDoc->getElementsByTagName ( "ValidarXmlResponse" )->item ( 0 ) == false ):
                echo '999;Erro na validação na prefeitura : nó ValidarXmlResponse não encontrado no xml ';
                exit;
            endif;
            ...
            if( $ValidarXmlResponse->getElementsByTagName ( "ValidarXmlResult" )->item ( 0 ) == false ):
                echo '999;Erro na validação na prefeitura : nó ValidarXmlResult não encontrado no xml ';
                exit;
            endif;
            ...
            if( $ValidarXmlResult->getElementsByTagName ( "MensagemRetorno" )->item ( 0 ) == false ):
                echo '999;Erro na validação na prefeitura : nó MensagemRetorno não encontrado no xml ';
                exit;
            endif;
            ...
            if( $MensagemRetorno->getElementsByTagName ( "tcMensagemRetorno" )->item ( 0 ) == false ):
                echo '999;Erro na validação na prefeitura : nó tcMensagemRetorno não encontrado no xml ';
                exit;
            endif;
            ...
            if( $tcMensagemRetorno->getElementsByTagName ( "Codigo" )->item ( 0 ) == false ):
                echo '999;Erro na validação na prefeitura : nó Codigo não encontrado no xml ';
                exit;
            endif;
            ...
            if ( $tcMensagemRetorno->getElementsByTagName ( "Codigo" )->item ( 0 )->nodeValue == 'I1' ):
                ...
                echo '999;Erro na validação na prefeitura :'. utf8_encode(trim($xmlRecDoc->getElementsByTagName ( "Mensagem" )->item ( 0 )->nodeValue));		
                exit;
            endif;
    ...
    public function enviar()
    {
        try{
            ...
            $xmlDoc = $this->criarXML( "RecepcionarXml" , true );

            $RecepcionarXml = $xmlDoc->getElementsByTagName ( "RecepcionarXml" )->item ( 0 ); 

            $metodo = $xmlDoc->createElement("metodo","RecepcionarLoteRps");
            $xml = $xmlDoc->createElement('xml');
            $xml->appendChild( $xmlDoc->createCDATASection($docxml));

            $RecepcionarXml->appendChild($metodo);
            $RecepcionarXml->appendChild($xml);

            $xml = $xmlDoc->saveXML($xmlDoc->documentElement);
            $xml = str_replace( array("\r\n", "\n", "\r"), '', $xml );

            $result = $this->startCurl( $xml , 'RecepcionarXml' );
            ...
            if( $RecepcionarXmlResponse->getElementsByTagName ( "RecepcionarXmlResult" )->item ( 0 ) == false ):
                echo '999;Erro na validação na prefeitura : nó ValidarXmlResult não encontrado no xml ';
                exit;
            endif;
