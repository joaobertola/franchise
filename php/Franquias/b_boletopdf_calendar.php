...
$dif = 5;
$dia = date('d-m-Y', strtotime('+3 days'));

for ($i = 1; $i <= $dif; $i++) {
    echo '<p class="hover-p"><a onclick="setValCalendar(\'' . date('Y-m-d', strtotime('+' . $i . ' day ' . $dia)) . '\',\'' . date('d/m/Y', strtotime('+' . $i . ' day ' . $dia)) . '\')"> Venc.: ' . date('d/m/Y', strtotime('+' . $i . ' day ' . $dia)) . '</a></p>';
}
?>
<style>
    .hover-p{color:#333;cursor:pointer;padding:5px;text-align:center;}
    .hover-p:hover{background:#EEE;}
</style>
<?php
/*
