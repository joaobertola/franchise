...
if ( $tipo == 'a' )
	...
	$command = "SELECT a.numdoc,CAST(MID(b.logon,1,6) AS UNSIGNED) as logon, c.fone, c.celular, c.nomefantasia, a.valor, a.vencimento,
			...
			inner join cs2.franquia f on c.id_franquia = f.id
			where $multa_nao a.debito is not null $situacao $intervalo $frq $cobr
			group by numdoc ";

if ( trim($situacao) <> 'and a.datapg is not null' ){
	$command .= "UNION
				 SELECT a.contrato,CAST(MID(b.logon,1,6) AS UNSIGNED) as logon, c.fone, c.celular, 
				 ...
				 INNER JOIN cs2.logon d     ON  a.codloja = d.codloja
				 INNER JOIN cs2.franquia f  ON  c.id_franquia = f.id
				 WHERE 1=1 $situacao $intervalo $frq $cobr ";
}

if ( trim($ordem) == 'a.vencimento asc' )
	$ordem = 'logon';
if ( trim($ordem) == 'c.razaosoc' )
	$ordem = 'razaosoc';
	
$command .="order by $ordem, vencimento";

$res = mysql_query($command,$con);
$linhas = mysql_num_rows($res);

$sql_cobradoras = " SELECT id, nome, funcao FROM cs2.funcionario
                    WHERE id_funcao = 12 AND ativo = 'S' AND id <> 90
                    ORDER BY Rand()";
...
if ( $atz_tabela == 'SIM' ){
	...
	for ( $i = 1 ; $i <= $qtd_cobradoras ; $i++ ){
		...
		<table align='center' border='0' cellpadding='0' cellspacing='0' class='bodyText' width='85%'>
			...
			$valor = $matriz['valor'];
			$vencimento = $matriz['vencimento2'];
			$referencia = $matriz['referencia'];
			if ( $referencia == 'MULTA' ) $ref = '(multa)';
			else if ( $referencia == 'ANTECIPACAO' ) $ref = '(Antecipacao)';
			else $ref = '';
			...
			echo "
					<td style='font-size:9px' 'width=300px'>$logon - $nomefantasia</td>
	...
	if ($linhas == "0")
	{
		...
		echo "<table align=\"center\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\" class=\"bodyText\" width='85%'>
			...
			<tr class=\"subtitulodireita\" style='font-size:9px' >";
			echo "	
		...
		while ($matriz = mysql_fetch_array($res))
		{
			...
			if ($datapg != "") {
				...
				if( $_SESSION["id"] == '163' ){
                                    if ( $referencia == 'ANTECIPACAO')
                                        $arqx = 'b_baixa_antecipa.php';
                                    else
                                        $arqx = 'b_baixa.php';
                                    
                                    echo "  <td align=\"center\">
                                                <a href=\"painel.php?pagina1=Franquias/$arqx&numdoc=$numdoc&situacao=$situacao&codigo1=$codigo1&codigo2=$codigo2&vencimento1=$vencimento1&vencimento2=$vencimento2&franqueado=$franqueado&cobranca=$cobranca&ordem=$ordem&lpp=$lpp&pagina=$pagina&periodo=$periodo\" onMouseOver=\"window.status='Quitar Titulo'; return true\">
