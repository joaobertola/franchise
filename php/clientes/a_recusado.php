...
if ($go=='ingressar') {
	if ($tpregistro == 1) {
		...
		$sql = "select MID(a.logon,1,LOCATE('S', a.logon) - 1) as logon, b.id_franquia, b.codloja, b.razaosoc from logon a
				...
				where MID(logon,1,LOCATE('S', logon) - 1)='$codigo' $frq";
	...
	$sql = "Select a.id, MID(c.logon,1,LOCATE('S', c.logon) - 1) as logon, b.razaosoc, date_format(a.data_msg,'%d/%m/%Y') as data_msg, a.titulo, a.data_leitura from cs2.correio_cliente a
...
if ($go=='individual') {
	$sql = "select a.codloja, date_format(a.data_msg,'%d/%m/%Y') as data_msg, a.titulo, a.mensagem, b.razaosoc, b.nomefantasia, b.insc, MID(c.logon,1,LOCATE('S', c.logon) - 1) as logon from correio_cliente a
			inner join cadastro b on a.codloja=b.codloja
