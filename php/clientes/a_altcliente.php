...
if ($go=='ingressar') {
	if (($tipo == "a") || ($tipo == "c")) {
		...
		$resulta = mysql_query("select b.codloja, a.logon, b.id_franquia from logon a
							inner join cadastro b on a.codloja=b.codloja
							where CAST(MID(a.logon,1,6) AS UNSIGNED)='$codigo' and id_franquia='{$_SESSION['usuario']}'", $con);
	}
	$linha = mysql_num_rows($resulta);
	if ($linha == 0) {
		print "<script>alert(\"Cliente não existe ou nao pertence a sua franquia!\"); javascript: history.back();</script>";
	} else {
		$matriz = mysql_fetch_array($resulta);
		$codloja = $matriz['codloja'];
		echo "<meta http-equiv=\"refresh\" content=\"0; url= painel.php?pagina1=clientes/a_altcliente1.php&codloja=$codloja\";>";
	}
}
?>